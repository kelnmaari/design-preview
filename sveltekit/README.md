...
| `src/routes/` | `/` library home, `/components` (+ `/forms`, `/data`, `/overlays`, `/content` sub-galleries, `/catalog` — every component with code), `/ui` (headless parts), `/nav/landing` + `/nav/internal` (navigation variants), `/tokens`, `/patterns`, `/tailwind`. Sidebar = one item per component. |
